...
const CLASS_DETAILS = [
  { title: "Class Duration", value: "45 Minutes", desc: "Structured 45 min core instruction + review" },
...
const FAQS = [
  {
    ...
    a: "Having a 61-key or 88-key weighted digital keyboard at home is helpful for daily practice, but not mandatory to start your first session. We also offer studio practice slot rentals for enrolled students.",
  },
  {
...
export default function PianoClient() {
  ...
  useEffect(() => {
    fetch("/api/teachers?department=Piano")
      .then((res) => res.ok ? res.json() : TEACHERS)
      .then((data) => setTeachers(data.length > 0 ? data : TEACHERS))
      .catch((err) => {
        console.error("Failed to load piano teachers:", err);
          ...
          <div className="relative bg-[#17140F] text-[#F7F2E7] overflow-hidden">
            <div className="relative z-10 px-8 py-16 sm:px-16 sm:py-20 text-center max-w-3xl mx-auto">
              ...
              <p className="text-[#B3A98F] text-base sm:text-lg mb-9 leading-relaxed">
                Experience a 1-on-1 consultation or enquire to discover the joy of music.
              </p>
